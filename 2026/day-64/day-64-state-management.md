...
- 3. It is an incremental no. given my terraform everytime tf creates new version of state file which helps in state locking and detects conflicts.
---
...
-> It says error acquiring state lock, shows lock info with details of who is managing state of resource. It is critical because multiple people working in the team might manage/modifying the state(by some terraform process) in the file at the same time causing conflicts. 
...
-> TF import adds existing resource to the state whereas creating a resource creates a new resource.
---
