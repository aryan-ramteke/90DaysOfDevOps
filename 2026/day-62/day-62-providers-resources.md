...
- version installed was 5.100.0
![alt text](image.png)
4. Read the provider lock file `.terraform.lock.hcl` -- what does it do?
- Its stores info/record about the provider plugins used. This file if included in vcs will make sure terraform selects the same plugin version by default when you run terraform init.

**Document:** What does `~> 5.0` mean? How is it different from `>= 5.0` and `= 5.0.0`?
~> 5.0 = means allow updates within same version group. It's like >= 5.0 and < 6.0.
---
...
-> TF detects this automatically because of implicit dependency. TF create dependency graph DAG before execution
- What would happen if you tried to create the subnet before the VPC existed?
-> If reference exists TF will not create it.  If we try to create all resources in parallel, tf will throw error that vpc is not created.
